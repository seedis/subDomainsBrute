...
class SubNameBrute:
    def __init__(self, target, options):
        ...
        if options.output:
            ...
            outfile_c = options.output+"_c.txt"
            #outfile_ip = options.output+"_ip.txt"
            #outfile_cip = options.output+"_cip.txt"
        else:
            ...
            #outfile_ip = target + '_ip'+ _name if not options.full_scan else target + '_full' + '_ip'+ _name
            outfile_c = target + '_c'+ _name if not options.full_scan else target + '_full' + '_c'+ _name
            #outfile_cip = target + '_cip'+ _name if not options.full_scan else target + '_full' + '_cip'+ _name
        self.outfile = open("result/"+outfile, 'w')
        #self.outfile_ip = open("result/"+outfile_ip, 'w')
        self.outfile_c = open("result/"+outfile_c, 'w')
        #self.outfile_cip=open("result/"+outfile_cip,'w')
        self.ip_dict = {}
        self.found_subs = set()
    ...
    def _load_dns_servers(self):
        ...
        self.dns_servers = []
        pool = Pool(30)
        for server in open('/Applications/subDomainsBrute/dict/dns_servers.txt').xreadlines():
            server = server.strip()
            if server:
    ...
    def _test_server(self, server):
        ...
        try:
            ...
            try:
                ...
                with open('/Applications/subDomainsBrute/bad_dns_servers.txt', 'a') as f:
                    f.write(server + '\n')
    ...
    def _load_sub_names(self):
        ...
        if self.options.full_scan:
            _file = '/Applications/subDomainsBrute/dict/subnames_full.txt'
        else:
    ...
    def _load_next_sub(self):
        ...
        self.next_subs = []
        _set = set()
        _file = '/Applications/subDomainsBrute/dict/next_sub_full.txt' if not self.options.full_scan else '/Applications/subDomainsBrute/dict/next_sub_full.txt'
        with open(_file) as f:
            for line in f:
                sub = line.strip()
        ...
        while not self.queue.empty():
            ...
            try:
                ...
                if answers:
                    ...
                    ips_array=ips.split(',')
                    flag1 = False
                    for i in range(len(ips_array)):
                        if len(all_ip)>0:
                            for mip in all_ip:
                                if ips_array[i] in all_ip:
                                    flag1 = True
                        if flag1 == True:
                            continue
                        all_ip.append(ips_array[i])
                        #ip去重处理输出到文件
                        # self.outfile_ip.write(ips_array[i]+'\n')
                        # self.outfile_ip.flush()

                        cip_array=ips_array[i].split('.')
                        cip_array_c=cip_array[:3]
                        flag2 = False
                        if len(cip)>0:
                            for nip in cip:
                                if cip_array_c in cip:
                                    flag2 = True
                        if flag2 == True:
                            continue 
                        cip.append(cip_array_c)
                        self.outfile_c.write(cip_array[0]+'.'+cip_array[1]+'.'+cip_array[2]+'.0\n')
                        # c段ip生成输出到文件
                        # for ipc in range(1,256):
                        #     self.outfile_cip.write(cip_array[0]+'.'+cip_array[1]+'.'+cip_array[3]+'.'+str(ipc)+"\n")
                        self.outfile_c.flush()
...
if __name__ == '__main__':
    ...
    parser.add_option('-f', dest='file', default='/Applications/subDomainsBrute/dict/subnames.txt',
